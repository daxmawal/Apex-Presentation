...
\begin{itemize}\itemsep0.3em
    ...
    \item Explores candidate configurations online
    \item Reuses converged configurations and can cache them in YAML
\end{itemize}
\end{beamerboxesrounded}
\end{frame}
...
\begin{frame}[fragile,t]{What Dyablo and APEX reports}
\begin{beamerboxesrounded}[upper=terminal title,lower=terminal body,width=\linewidth,shadow=false]{%
\textcolor{red!70}{\footnotesize\textbullet}\hspace{0.15em}%
\textcolor{yellow!80}{\footnotesize\textbullet}\hspace{0.15em}%
\textcolor{green!70!black}{\footnotesize\textbullet}\hspace{0.55em}%
...
\begin{semiverbatim}
\textcolor{hotspotBlue}{APEX Version: -bd4b707-fix/cuda-nvml-gpu-timers}
\textcolor{hotspotBlue}{KOKKOS CONFIG}
\textcolor{hotspotBlue}{  Kokkos Version: 5.0.0}
\textcolor{hotspotBlue}{  Default Device: Cuda}
\textcolor{hotspotBlue}{  GPU architecture: ADA89}
----------------------------------------
...
\end{beamerboxesrounded}
\vspace{0.2em}
{\scriptsize \textcolor{hlOrange}{Dyablo} timer hierarchy exported through regions. \textcolor{hotspotBlue}{APEX} runtime banner, NVML counters, and CPU timer summary.}
\end{frame}
...
\textcolor{yellow!80}{\footnotesize\textbullet}\hspace{0.15em}%
\textcolor{green!70!black}{\footnotesize\textbullet}\hspace{0.55em}%
\texttt{\scriptsize Dyablo timer hierarchy exported through Kokkos regions}}
{\ttfamily\tiny\color{termText}
\begin{semiverbatim}
...
\end{beamerboxesrounded}
\vspace{0.2em}
{\scriptsize This hierarchy is produced by Dyablo's own timers. APEX can observe the same structure because Dyablo exports these phases through \texttt{Kokkos::Profiling::pushRegion()/popRegion()}.}
...
\begin{frame}[fragile,t]{APEX observations on Dyablo run}
\small
\begin{beamerboxesrounded}[upper=terminal title,lower=terminal body,width=\linewidth,shadow=false]{%
\textcolor{red!70}{\footnotesize\textbullet}\hspace{0.15em}%
\textcolor{yellow!80}{\footnotesize\textbullet}\hspace{0.15em}%
...
\scriptsize
{\color{ceaGreen}\textbf{APEX learns online across repeated visits to the same context during a Dyablo run.}}
...
\tiny
\raggedright
A context is one tunable Kokkos execution context.
...
\raggedright
\texttt{window = 5}\\
allow a tuning update after at least 5 observations

\vspace{0.05em}
\texttt{Converged = true}\\
reuse later in the run and from YAML cache in later runs

\vspace{0.05em}
\texttt{Converged = false}\\
not reused from the YAML cache, a later run starts tuning that context again
\end{beamerboxesrounded}
\end{columns}
\end{frame}
